...
{ 
    ...
    for (unsigned i = 0; i < scenes.size(); ++i) { 
        ...
        if (scenes[i].intersect(rayorig, raydir, t0, t1)) { 
            if (t0 < 0) t0 = t1; 
            if (t0 < tnear) { 
                tnear = t0; 
                scene = &scenes[i]; 
    ...
    if (!scene) return Vec3f(2); 
    Vec3f surfaceColor = 0; // color of the ray/surfaceof the object intersected by the ray 
    Vec3f phit = rayorig + raydir * tnear; // point of intersection 
    Vec3f nhit = phit - scene->center; // normal at the intersection point 
    ...
    if ((scene->transparency > 0 || scene->reflection > 0) && depth < MAX_RAY_DEPTH) { 
        ...
        surfaceColor = ( 
            ...
            refraction * (1 - fresneleffect) * scene->transparency) * scene->surfaceColor; 
    } 
    else { 
        // it's a diffuse object, no need to raytrace any further
        for (unsigned i = 0; i < scenes.size(); ++i) { 
            if (scenes[i].emissionColor.x > 0) { 
                // this is a light
                Vec3f transmission = 1; 
                Vec3f lightDirection = scenes[i].center - phit; 
                lightDirection.normalize(); 
                for (unsigned j = 0; j < scenes.size(); ++j) { 
                    if (i != j) { 
                        float t0, t1; 
                        if (scenes[j].intersect(phit + nhit * bias, lightDirection, t0, t1)) { 
                            transmission = 0; 
                            break; 
                        } 
                    } 
                } 
                surfaceColor += scene->surfaceColor * transmission * 
                std::max(float(0), nhit.dot(lightDirection)) * scenes[i].emissionColor; 
            } 
        } 
    } 
 
    return surfaceColor + scene->emissionColor; 
} 
